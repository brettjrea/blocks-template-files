...
import { FileBlockProps, getLanguageFromFilename } from "@githubnext/blocks";                                   import { Button, Box } from "@primer/react";
import "./index.css";                                   import plantumlEncoder from 'plantuml-encoder';
...
};                                                      export interface ExampleFileBlockProps extends FileBlockProps {                                                   content: string;
}                                                       
export default function ExampleFileBlock(props: ExampleFileBlockProps) {
  ...
  const language = Boolean(context.path)                    ? getLanguageFromFilename(context.path)
    : "N/A";                                            
  return (
    <Box p={4}>
      ...
      >
        <Box                                                      bg="canvas.subtle"
          p={3}                                                   borderBottomWidth={1}
          borderBottomStyle="solid"                               borderColor="border.default"
        >                                                         File: {context.path} {language}
          ...
          <Button                                                   onClick={() =>
            ...
            {metadata.number || 0} times                          </Button>
          <pre className="mt-3 p-3">{content}</pre>               {language === "plantuml" && (                             <ReactPlantUML src={content} alt="PlantUML diagram" />                                                        )}
        </Box>
      </Box>
    </Box>                                                );
}                                                       
interface CommonBlockProps {
  block: {
    ...
    description: string;                                    entry: string;
    matches?: string[];                                   };
  context: {                                                path: string;
    file: string;                                           folder: string;
    repo: string;                                           owner: string;
    sha: string;                                          };
  metadata: any;                                          onUpdateMetadata: (newMetadata: any) => void;
